...
There's client versions of Pinepods for Linux (Deb, RPM, AppImage, Flatpak, AUR), Mac, Windows, Android, and iOS. Scroll down for install instructions. 
...
Grab the latest linux release. There's an appimage, a deb, and an rpm. Use the appimage if you aren't using a debian or red hat based distro. Change the permissions if using the appimage version to allow it to run.
